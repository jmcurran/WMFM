...
testthat::test_that("lmToExplanationPrompt gives logistic factor comparison safeguards", {
  ...
  testthat::expect_match(prompt, "do not write that the factor raises, lowers, increases, decreases, or changes", fixed = TRUE)
  testthat::expect_match(prompt, "fitted probabilities are different but the interval is wide enough", fixed = TRUE)
  testthat::expect_match(prompt, "Do not combine a fitted-probability comparison with an uncertain odds-ratio estimate", fixed = TRUE)
