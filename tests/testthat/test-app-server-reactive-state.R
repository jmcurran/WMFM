...
testthat::test_that("fit-model observers isolate and store model follow-up question state", {
  ...
  testthat::expect_match(fitModelText, "attr(m, \"wmfm_model_followup_question\") = followupQuestion", fixed = TRUE)
  testthat::expect_match(fitModelText, "attr(m, \"wmfm_model_followup_payload\") = list(", fixed = TRUE)
})
