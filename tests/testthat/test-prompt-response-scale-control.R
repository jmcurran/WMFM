testthat::test_that("lmToExplanationPrompt includes logistic response-scale control", {
  ...
  testthat::expect_match(prompt, "Use probabilities for fitted values, baselines, and group-level outcomes.", fixed = TRUE)
  testthat::expect_match(prompt, "Use odds multipliers or odds ratios only for effects and direct comparisons.", fixed = TRUE)
