...
testthat::test_that("GLM interval follow-ups return mean confidence intervals while flagging unsupported prediction intervals", {
  df = data.frame(Y = c(1, 2, 4, 2, 3, 5, 6, 4), X = c(1, 2, 3, 4, 2, 5, 6, 3))
  model = stats::glm(Y ~ X, data = df, family = stats::poisson())

  out = computeGlmModelQuestionPrediction(model, "Predict Y when X = 3 with confidence interval and prediction interval")
  testthat::expect_identical(out$status, "ok")
  testthat::expect_identical(out$glmFamily, "poisson")
