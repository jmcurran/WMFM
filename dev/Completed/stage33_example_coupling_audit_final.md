...
Generated at: Wed Jun  3 08:46:26 NZST 2026
Branch: example-coupling-audit

## Git status

```text
?? stage33_example_coupling_audit_final.md
```
...
R/model-question-followup-answer.R:281:#' Build deterministic follow-up failure answer text
R/model-question-followup-answer.R:285:#' @return Character scalar answer text, or empty string when no failure should
R/model-question-followup-answer.R:289:buildDeterministicFollowupFailureAnswer = function(prediction) {
R/model-factorOnly.R:22:#' @param hcType Sandwich type passed to \code{sandwich::vcovHC()} (e.g. \code{"HC0"}, \code{"HC3"}).
R/model-glm-notation.R:47:      failureLevel = as.character(levels(response)[1])
R/model-glm-notation.R:54:        failureLevel = observed[1]
R/model-glm-notation.R:57:        failureLevel = "0"
R/model-glm-notation.R:65:      failureLevel = failureLevel,
R/model-glm-notation.R:67:      probabilityFailure = paste0("Pr(", responseName, " = \"", failureLevel, "\")"),
R/model-glm-notation.R:68:      oddsSuccess = paste0("Odds(", responseName, " = \"", successLevel, "\" vs ", responseName, " = \"", failureLevel, "\")"),
R/model-glm-notation.R:69:      oddsFailure = paste0("Odds(", responseName, " = \"", failureLevel, "\" vs ", responseName, " = \"", successLevel, "\")"),
...
R/model-explanation-cleanText.R:1130:#' @param audit Optional explanation audit object passed to
...
R/model-question-classifier.R:175:      "\\bwhat\\b.*\\b(frequency|count|number|probability|odds|chance|value|response)\\b.*\\b(expect|expected)\\b",
R/model-question-classifier.R:176:      "\\bwhat\\b.*\\b(expect|expected)\\b.*\\b(frequency|count|number|probability|odds|chance|value|response)\\b",
R/model-question-classifier.R:213:  if (grepl("\\b(effect size|magnitude|how big|size of the effect|size of effect)\\b", normalizedText, perl = TRUE)) {
...
R/model-binomial-outcomes.R:5:#' labels for the success and failure outcomes so the app can write labels
R/model-binomial-outcomes.R:10:#' @return A list with `successLabel`, `failureLabel`, and `responseName`.
R/model-binomial-outcomes.R:21:      failureLabel = as.character(levels(y)[1]),
R/model-binomial-outcomes.R:29:      failureLabel = "FALSE",
R/model-binomial-outcomes.R:40:        failureLabel = as.character(values[1]),
R/model-binomial-outcomes.R:48:    failureLabel = "Failure",
R/model-binomial-outcomes.R:56:#' @param outcome Either `"success"` or `"failure"`.
R/model-binomial-outcomes.R:61:formatBinomialProbabilityLabel = function(model, outcome = c("success", "failure"), indexed = FALSE) {
R/model-binomial-outcomes.R:65:  outcomeLabel = if (identical(outcome, "success")) labels$successLabel else labels$failureLabel
R/model-binomial-outcomes.R:78:#' @param outcome Either `"success"` or `"failure"`.
R/model-binomial-outcomes.R:83:formatBinomialOddsLabel = function(model, outcome = c("success", "failure"), indexed = FALSE) {
R/model-binomial-outcomes.R:87:  outcomeLabel = if (identical(outcome, "success")) labels$successLabel else labels$failureLabel
...
tests/testthat/test-domain-agnostic-behaviour.R:31:testthat::test_that("generic prompt examples avoid course-specific outcome wording", {
tests/testthat/test-domain-agnostic-behaviour.R:34:  testthat::expect_no_match(promptText, "Pass", fixed = TRUE)
tests/testthat/test-domain-agnostic-behaviour.R:40:testthat::test_that("follow-up prediction classification does not require course wording", {
tests/testthat/test-domain-agnostic-behaviour.R:58:  testthat::expect_no_match(sourceText, "Pr(Pass", fixed = TRUE)
tests/testthat/test-domain-agnostic-behaviour.R:59:  testthat::expect_no_match(sourceText, "Odds(Pass", fixed = TRUE)
...
tests/testthat/test-postProcessExplanationText.R:19:    "If magnitude increases by one unit, the expected count is multiplied by 0.21."
...
tests/testthat/test-postProcessExplanationText.R:161:    "If magnitude increases by one unit, the expected count is multiplied by 0.21."
...
man/getBinomialOutcomeLabels.Rd:18:labels for the success and failure outcomes so the app can write labels
...
NEWS.md:22:- Preserve setting-generic wording without reintroducing course-specific prompt rules.
NEWS.md:41:- Replace course-specific equation-prompt examples with neutral symbolic examples.
NEWS.md:43:- Remove course-specific follow-up detection cues from production question handling.
NEWS.md:51:- Generalize deterministic parser regression coverage so anti-coupling checks no longer depend on earthquake or diamond examples.
NEWS.md:57:- Replace magnitude-specific unit-change cleanup with predictor-agnostic unit-change phrasing.
NEWS.md:69:- Refined log-log unit-change follow-up wording so fixed carat changes keep the reference point and use carat units.
NEWS.md:73:- Allowed the log-log percentage-language post-processing rule to bypass numeric-token preservation while keeping the guard active for other rules.
NEWS.md:121:- Kept summary text student-facing and separate from diagnostic pass/fail language.
NEWS.md:147:- Restore password protection for the maintainer-only developer-mode toggle while preserving the local developer-mode preference after a successful unlock.
NEWS.md:196:- Change the runner workflow so failed build attempts still consume the next `0.2.5.xxx` build number.
NEWS.md:204:- Harden Ollama discovery so failures are handled gracefully and tests do not require a real Ollama service.
NEWS.md:211:- Add and refine log-log model support, including power-law interpretation and Diamonds II, Diamonds III, and Diamonds IV examples.
```
