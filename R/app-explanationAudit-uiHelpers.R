...
#' @param dataDescription Optional character scalar.
#'
#' @return A Shiny UI object.
#' @keywords internal
#' @importFrom htmltools tagList tags
renderExplanationTutorUi = function(
    text = NULL,
    available = TRUE,
  ...
  dataDescriptionText = trimws(as.character(dataDescription %||% ""))

  if (nzchar(researchQuestionText)) {
    introNodes[[length(introNodes) + 1]] = tags$p(
  ...
  if (nzchar(dataDescriptionText)) {
    introNodes[[length(introNodes) + 1]] = renderTeachingSummaryText(dataDescriptionText)
  }

  introNodes[[length(introNodes) + 1]] = renderTeachingSummaryText(text)

  do.call(
    tags$div,
    c(
      list(class = "wmfm-explanation-helper-box"),
      introNodes
    )
  )
}
