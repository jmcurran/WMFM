...
extractWmfmSemanticEvidence = function(explanationText, modelInfo = list()) {
  ...
  list(
    ...
    effectMagnitude = extractFirstNumber(
      "(effect|increase|raises?|higher|additional|extra|difference|approximately|about|predicted|expected|multiplier|double[sd]?|multiplied)[^0-9-+]{0,60}[-+]?[0-9]+([.][0-9]+)?"
    ),
    ...
    comparisonMentioned = detectPattern("female|male|boys|girls|versus|compared|between|groups|similarly|difference|in comparison|higher than|lower than"),
    individualPredictionMentioned = detectPattern("predicted (exam )?mark|predicted value|prediction for (a|one|this|new) student|individual prediction"),
    predictionIntervalMentioned = detectPattern("prediction interval"),
    expectedResponseMentioned = detectPattern("expected (mark|value|response|mean|number|count)|average (mark|value|response)"),
    confidenceIntervalForMeanMentioned = detectPattern("expected[^.;]{0,100}confidence interval|average[^.;]{0,100}confidence interval|confidence interval[^.;]{0,100}(expected|average)"),
    predictionMeanDistinctionMentioned = detectPattern("narrower because[^.;]{0,120}(average|mean)|not a prediction for a new|different question to how well.*average|prediction interval[^.;]{0,180}confidence interval"),
...
buildWmfmSemanticEvidenceDiagnostics = function(evidence) {
  ...
  fieldOrder = c(
    ...
    "comparisonMentioned",
    "individualPredictionMentioned",
    "predictionIntervalMentioned",
    "expectedResponseMentioned",
    "confidenceIntervalForMeanMentioned",
    "predictionMeanDistinctionMentioned",
  ...
  labelMap = c(
    ...
    individualPredictionMentioned = "Individual prediction mentioned",
    predictionIntervalMentioned = "Prediction interval mentioned",
    expectedResponseMentioned = "Expected response mentioned",
    confidenceIntervalForMeanMentioned = "Confidence interval for mean mentioned",
    predictionMeanDistinctionMentioned = "Prediction and mean distinguished",
  ...
  detailMap = c(
    ...
    individualPredictionMentioned = "An individual or future-outcome prediction is stated.",
    predictionIntervalMentioned = "A prediction interval is stated.",
    expectedResponseMentioned = "An expected response or mean response is stated.",
    confidenceIntervalForMeanMentioned = "A confidence interval is attached to an expected or mean response.",
    predictionMeanDistinctionMentioned = "The explanation distinguishes uncertainty for an individual prediction from uncertainty about a mean response.",
