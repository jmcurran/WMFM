...
buildExplanationPromptDiagnosticsUi = function(diagnostics = NULL) {
  ...
  diagnosticsBundle = paste(
    ...
    "",
    "Effect scale:", as.character(deterministic$effectScale %||% "not_applicable"),
    "",
    "GLM family/link:", paste(
      as.character(deterministic$glmFamily %||% prediction$glmFamily %||% "not_applicable"),
      as.character(deterministic$glmLink %||% prediction$glmLink %||% "not_applicable"),
      sep = "/"
    ),
    "",
    "Response scale:", as.character(prediction$responseScale %||% "not_applicable"),
    "",
    "Requested unit-change predictor:", as.character(unitChange$predictorName %||% "not_applicable"),
    "",
    "Requested unit change:", paste(capture.output(str(unitChange$requestedUnitChange %||% NULL)), collapse = "\n"),
    "",
    "Original one-unit effect:", paste(capture.output(str(unitChange$oneUnitEffect %||% NULL)), collapse = "\n"),
    "",
    "Transformed unit-change effect:", paste(capture.output(str(unitChange$transformedEstimate %||% unitChange$unitChangeEffect %||% NULL)), collapse = "\n"),
    "",
    "Transformed unit-change interval:", paste(capture.output(str(unitChange$confidenceInterval %||% list())), collapse = "\n"),
    "",
...
buildExplanationPromptDiagnosticsJson = function(diagnostics = NULL) {
  ...
  out = list(
    ...
    glmFamily = as.character(deterministic$glmFamily %||% prediction$glmFamily %||% ""),
    glmLink = as.character(deterministic$glmLink %||% prediction$glmLink %||% ""),
    responseScale = as.character(prediction$responseScale %||% ""),
    responseDescription = as.character(prediction$responseDescription %||% ""),
    effectScale = as.character(deterministic$effectScale %||% ""),
    warnings = deterministic$warnings %||% prediction$warnings %||% character(0),
    ...
    extrapolationStatus = as.character(prediction$extrapolationPolicy$status %||% prediction$extrapolationDiagnostics$status %||% ""),
    extrapolationExplanation = as.character(prediction$extrapolationExplanation %||% prediction$extrapolationDiagnostics$explanationText %||% ""),
    extrapolationDiagnostics = prediction$extrapolationDiagnostics %||% prediction$extrapolationPolicy$diagnostics %||% list(),
    ...
    originalUnitChangeEffect = unitChange$oneUnitEffect %||% NULL,
    transformedUnitChangeEffect = unitChange$transformedEstimate %||% unitChange$unitChangeEffect %||% NULL,
    transformedUnitChangeInterval = unitChange$confidenceInterval %||% NULL,
    unitChangeModelType = as.character(unitChange$modelType %||% ""),
