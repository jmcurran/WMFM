...
plotWmfmExplanationClaimHeatmap = function(
  ...
  colorMap = getWmfmClaimColorMap()
  missingValues = setdiff(legendValues, names(colorMap))

  if (length(missingValues) > 0) {
    fallbackCols = grDevices::gray.colors(length(missingValues), start = 0.85, end = 0.35)
    names(fallbackCols) = missingValues
    colorMap = c(colorMap, fallbackCols)
  }

  colorMap = colorMap[legendValues]
