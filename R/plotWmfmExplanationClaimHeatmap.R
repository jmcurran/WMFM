...
#' claim fields only. This function is intended for visualising run-to-run
#' variation in interpretation patterns and no longer supports judged fields or
#' aggregate scores.
#'
#' Fields are ordered by purity, and runs are ordered by overall claim-profile
#' purity relative to the modal value in each field. Missing values are shown as
#' white tiles with an X overlay.
#'
...
#'
#' @importFrom ggplot2 aes element_text geom_segment geom_tile ggplot labs scale_fill_manual theme theme_minimal
    ...
    if (nrow(missingDf) > 0) {
        plotObj = plotObj +
            geom_segment(
                data = missingDf,
                aes(
                    x = as.numeric(runId) - 0.42,
                    xend = as.numeric(runId) + 0.42,
                    y = as.numeric(fieldLabel) - 0.42,
                    yend = as.numeric(fieldLabel) + 0.42
                ),
                inherit.aes = FALSE,
                linewidth = 0.4,
                colour = "#4D4D4D"
